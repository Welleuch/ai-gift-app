...
const ModelViewer = dynamic(() => import('../components/ModelViewer'), {
    ssr: false,
    loading: () => < div className = "text-slate-400 animate-pulse font-bold text-xs" > Loading 3 D Engine... < /div>
});
...
export default function Home() {
    ...
    const [messages, setMessages] = useState([{
        role: 'assistant',
        content: "Welcome to the AI 3D Gift Studio. Tell me about the recipient's hobbies to begin."
    }]);
        ...
        try {
            ...
            if (res.data.visual_prompt) {
                ...
                const genRes = await axios.post('http://localhost:8000/api/generate-images', {
                    visual_prompt: res.data.visual_prompt
                });
...
const handlePrepareGCode = async () => {
  ...
  setLoading(false);
    ...
    return ( <
        ...
        <
        div className = "bg-blue-500 p-2 rounded-lg text-white" > < Sparkles size = {
            20
        }
        /></div >
        <
        ...
        className = "flex-1 p-6 overflow-y-auto space-y-6" > {
            messages.map((m, i) => ( <
                ...
                <
                div className = {
                    `p-4 rounded-2xl max-w-[85%] text-sm shadow-sm ${m.role === 'user' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-200'}`
                } > {
                    m.content
                } < /div> <
                /div>
            ...
            loading && < div className = "text-blue-400 text-xs animate-pulse flex items-center gap-2" > < Loader2 className = "animate-spin"
            size = {
                12
            }
            /> {status}</div >
        ...
        div className = "flex-1 relative flex flex-col bg-[#f8fafc]" > {
            showPedestalUI && < PedestalControls settings = {
                pedestalSettings
            }
            setSettings = {
                setPedestalSettings
            ...
            main className = "flex-1 p-12 flex items-center justify-center" > {
                !modelUrl && generatedImages.length === 0 && !loading && < Box size = {
                    80
                }
                className = "text-slate-200" / >
            } {
                loading && generatedImages.length === 0 && !modelUrl && < div className = "text-center" > < Loader2 size = {
                    40
                }
                className = "animate-spin text-blue-500 mx-auto mb-4" / > < h2 className = "text-xl font-bold" > {
                    status
                } < /h2></div >
            }
                ...
                !modelUrl && generatedImages.length > 0 && ( <
                    div className = "grid grid-cols-2 gap-10 w-full max-w-5xl" > {
                        generatedImages.map((img, i) => ( <
                            ...
                            className = "cursor-pointer rounded-[40px] overflow-hidden shadow-2xl border-8 border-white hover:scale-105 transition-all" >
                            <
                            img src = {
                                img
                            }
...

            {
                modelUrl && ( <
                    ...
                    /> {showPedestalUI ? 'HIDE SETTINGS' : 'CUSTOMIZE BASE'} <
                    /button> <
                    button onClick = {
                        handlePrepareGCode
                    }
                    className = "bg-blue-600 text-white px-8 py-4 rounded-2xl font-bold shadow-xl" > PREPARE G - CODE < /button> <
                    /div> <
                    /div> <
                    /div>
                )
...

            {
                ...
                orderSummary && !isOrdered && ( <
                    ...
                    className = "flex-[2] bg-blue-600 text-white py-4 rounded-2xl font-black" > PAY & SHIP🚀 < /button> <
                    /div> <
                    /div> <
                    /div>
                )
...

            {
                isOrdered && ( <
                    ...
                    className = "text-green-500 mx-auto mb-8" / >
                    <
                    ...
                    className = "text-blue-600 font-bold border-b-2 border-blue-600" > New Design < /button> <
                    /div> <
                    /div>
                )
            } <
